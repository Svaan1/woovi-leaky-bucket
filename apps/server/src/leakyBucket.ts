...
export class LeakyBucketService {
    ...
    async leakTokens(tokensToConsume: number): Promise<boolean> {
        try {
            if (tokensToConsume <= 0) {
                throw new LeakyBucketError('Tokens to consume must be a positive number');
            }
            ...
            if (tokensToRefill > 0) {
                bucket.tokens = Math.min(bucket.tokens + tokensToRefill, this.config.maxTokens);
            ...
            throw new LeakyBucketError(`Failed to consume tokens: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async refundTokens(tokensToRefund: number): Promise<void> {
        try {
            if (tokensToRefund <= 0) {
                throw new LeakyBucketError('Tokens to refund must be a positive number');
            }
            
            const bucket = await this.fetchBucket();
        
            if (!bucket) {
                throw new LeakyBucketError(`Cannot refund tokens: Bucket for user ${this.userId} does not exist.`);
            }
        
            bucket.tokens = Math.min(bucket.tokens + tokensToRefund, this.config.maxTokens);
            await this.saveBucket(bucket);
            ...
            throw new LeakyBucketError(`Failed to refund tokens: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
